...
const I18N = {
  RU: {
    ...
    adbStep3ShizukuDesc: "Запустите службу <b>Shizuku</b> по беспроводной отладке (Wi-Fi), откройте <b>Termux</b> и запустите скрипт <a href=\"shizuku_sync.sh\" download=\"shizuku_sync.sh\" class=\"shizuku-dl-link\"><code>📥 Скачать shizuku_sync.sh</code></a> (или откройте <a href=\"https://github.com/Onarous/Guild-of-Monster-Girls-Encyclopedia/blob/main/shizuku_sync.sh\" target=\"_blank\" class=\"shizuku-dl-link\">исходник на GitHub</a>).<br>⚠️ <b>Важное примечание:</b> На большинстве физических телефонов без Root доступ к папке игры блокируется ядром Android (ошибка <code>Permission denied</code>). Если этот метод не сработал на вашем устройстве, используйте 100% рабочий способ — вкладку <b>«🎫 Вход по токену»</b> через мобильный сниффер трафика (Reqable/HttpCanary) или эмулятор на ПК.",
  ...
  EN: {
    ...
    adbStep3ShizukuDesc: "Start <b>Shizuku</b> via Wireless Debugging, open <b>Termux</b> and run <a href=\"shizuku_sync.sh\" download=\"shizuku_sync.sh\" class=\"shizuku-dl-link\"><code>📥 Download shizuku_sync.sh</code></a> (or view <a href=\"https://github.com/Onarous/Guild-of-Monster-Girls-Encyclopedia/blob/main/shizuku_sync.sh\" target=\"_blank\" class=\"shizuku-dl-link\">Source on GitHub</a>).<br>⚠️ <b>Note:</b> On unrooted Android devices, the OS may restrict private app access (<code>Permission denied</code>). If this happens, please use the 100% reliable <b>«🎫 Token Tab»</b> with a mobile traffic sniffer (Reqable / HttpCanary) or PC emulator.",
  ...
  CN: {
    ...
    adbStep3ShizukuDesc: "通过无线调试启动 <b>Shizuku</b>，在 <b>Termux</b> 中运行 <a href=\"shizuku_sync.sh\" download=\"shizuku_sync.sh\" class=\"shizuku-dl-link\"><code>📥 下载 shizuku_sync.sh</code></a> (也可查看 <a href=\"https://github.com/Onarous/Guild-of-Monster-Girls-Encyclopedia/blob/main/shizuku_sync.sh\" target=\"_blank\" class=\"shizuku-dl-link\">GitHub 源码</a>)。<br>⚠️ <b>注意：</b> 在大多数无 Root 的实体手机上，Android 系统权限将拦截读取 (<code>Permission denied</code>)。如遇到此情况，请使用 100% 稳定的 <b>«🎫 Token 登录»</b> 配合抓包工具 (Reqable/HttpCanary) 或电脑模拟器。",
